...
public class PhoneBookTest extends BaseStageTest {
    ...
    public CheckResult check(String reply, Object clue) {
        ...
        if (stat1.size() + stat2.size() < 2) {
            return CheckResult.FALSE("Your output should contain twice the phrase `500 / 500`");
        }
        
        
        List<String> timestamps = findAll(reply, timeRegex);
        if (timestamps.size() != 4) {
            return CheckResult.FALSE("Your output should contain 4 timer outputs, but found "
                    + timestamps.size());
        }
        // should not fail..
        ...
        if (reply.toLowerCase().contains("stopped")) {
            if (t3 < t1) {
                return CheckResult.FALSE("You printed `stopped`, " +
